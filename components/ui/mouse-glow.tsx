...
export function MouseGlow() {
  ...
  return (
    <>
      ...
      />
      <motion.div
        className="pointer-events-none fixed top-0 left-0 w-3 h-3 bg-saffron rounded-full z-[100] mix-blend-screen shadow-[0_0_10px_rgba(244,160,36,0.8)]"
        animate={{ x: mousePosition.x - 6, y: mousePosition.y - 6 }}
        transition={{ type: "tween", ease: "backOut", duration: 0.05 }}
      />
      <motion.div
        className="pointer-events-none fixed top-0 left-0 w-10 h-10 border border-saffron/40 rounded-full z-[99] mix-blend-screen"
